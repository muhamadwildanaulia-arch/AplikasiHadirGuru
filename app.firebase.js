...
/* ----------------- Admin: robust handler tambah guru (DITAMBAHKAN DI SINI) ----------------- */
(function setupAdminAddGuruHandlerRobust(){
  // attach after DOM ready
  document.addEventListener('DOMContentLoaded', () => {
    try {
      const btnSave = document.getElementById('guru-save');
      const btnCancel = document.getElementById('guru-cancel');
      const modal = document.getElementById('guru-modal');

      if (!btnSave) {
        console.warn('setupAdminAddGuruHandlerRobust: #guru-save tidak ditemukan di DOM.');
        return;
      }

      function closeModalAndClear(){
        if (modal) modal.classList.add('hidden');
        const nipEl = document.getElementById('guru-nip');
        const namaEl = document.getElementById('guru-nama-input');
        const jabEl = document.getElementById('guru-jabatan-input');
        if (nipEl) nipEl.value = '';
        if (namaEl) namaEl.value = '';
        if (jabEl) jabEl.value = '';
      }

      if (btnCancel) btnCancel.addEventListener('click', (e) => { e.preventDefault(); closeModalAndClear(); });
      ...
      btnSave.addEventListener('click', async function(e){
        ...
        const jabatan = jabEl ? jabEl.value.trim() : '';

        if (!nama) {
          try { window.toast && window.toast('Nama guru harus diisi', 'err'); } catch(e){}
          return;
        }

        // load current local list for duplicate check
        let localList = [];
        try { localList = JSON.parse(localStorage.getItem(KEY_GURU) || '[]'); } catch(e){ localList = []; }

        // simple duplicate check: same NIP (if provided) or same nama (case-insensitive)
        if (nip) {
          const dupNip = localList.find(g => g.nip && String(g.nip).trim() === String(nip).trim());
          if (dupNip) {
            window.toast && window.toast('NIP sudah terdaftar', 'err');
            return;
          }
        }
        const dupNama = localList.find(g => g.nama && String(g.nama).trim().toLowerCase() === String(nama).trim().toLowerCase());
        if (dupNama) {
          window.toast && window.toast('Nama guru sudah terdaftar', 'err');
          return;
        }

        const newGuru = { nip, nama, jabatan, status: 'Aktif' };
        const origHtml = btnSave.innerHTML;
        btnSave.disabled = true;
        btnSave.innerHTML = 'Menyimpan...';

        try {
          // Prefer Firebase if available and db initialized
          if (typeof window.addGuruFirebase === 'function' && typeof db !== 'undefined' && db) {
            try {
              const newKey = await window.addGuruFirebase(newGuru);
              // update local cache
              try {
                const cur = JSON.parse(localStorage.getItem(KEY_GURU) || '[]');
                cur.push(Object.assign({ id: newKey }, newGuru));
                saveGuruListToLocal(cur);
                if (typeof renderGuruUi === 'function') renderGuruUi(cur);
                window.dispatchEvent(new CustomEvent('gurus-updated', { detail: cur }));
              } catch(e){ console.warn('update local cache after firebase add failed', e); }
              window.toast && window.toast('Guru tersimpan di server', 'ok');
              console.log('Guru tersimpan di Firebase dengan key:', newKey);
            } catch(err){
              console.warn('addGuruFirebase gagal — akan coba simpan lokal', err);
              throw err;
            }
          } else {
            ...
            cur.push(Object.assign({ id }, newGuru));
            saveGuruListToLocal(cur);
            if (typeof renderGuruUi === 'function') renderGuruUi(cur);
            window.dispatchEvent(new CustomEvent('gurus-updated', { detail: cur }));
            window.toast && window.toast('Guru disimpan (lokal)', 'info');
            console.log('Guru disimpan lokal dengan id:', id);
          }
          closeModalAndClear();
        } catch(err){
          // if firebase failed, attempt local save as safe fallback
          try {
            const cur = JSON.parse(localStorage.getItem(KEY_GURU) || '[]');
            const id = 'local_' + Date.now();
            cur.push(Object.assign({ id }, newGuru));
            ...
            if (typeof renderGuruUi === 'function') renderGuruUi(cur);
            window.dispatchEvent(new CustomEvent('gurus-updated', { detail: cur }));
            window.toast && window.toast('Terjadi masalah server — guru disimpan (lokal)', 'err');
            console.warn('Falling back to local save after error', err);
            closeModalAndClear();
          } catch(inner){
            window.toast && window.toast('Gagal menyimpan guru', 'err');
            console.error('save guru error (both firebase & local failed)', inner);
          }
        } finally {
      ...
      console.log('setupAdminAddGuruHandlerRobust: handler siap.');
    } catch(err){
      console.error('setupAdminAddGuruHandlerRobust: error saat pasang handler', err);
    }
  });
})();
